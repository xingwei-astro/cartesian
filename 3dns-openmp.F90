...
! ifort -o 3dns 3dns-openmp.F90 -I$HOME/fftw/include/ -L$HOME/fftw/lib/ -lfftw3 -lm -qopenmp -lfftw3_threads -lpthread -lfftw3_omp
...
! dynamo driven by helical forcing, field normalized with sqrt(rho*mu)*Omega*l
...
double precision Re, Ro, Va, Rm, Ek, Pm
parameter (Re=1.d3, Ro=1.d-1, Va=1.d-1, Rm=1.d3, Ek=1.d-3, Pm=1.d0)
...
parameter (af=5.d-3, kf1=1, kf2=1, kf3=1, freq=-1.175d0)
...
integer void
double precision s_time, e_time
...
!$omp parallel do private(j3,j2,j1) schedule(dynamic)
do j3=1,n3
 do j2=1,n2
  do j1=1,n1
#ifdef flow
...
#endif
!$omp parallel do private(k3,k2,k1,kk,w) schedule(dynamic)
do k3=1,n3
 do k2=1,n2
  do k1=1,nc
...
#endif
#ifdef field
   a1(k1,k2,k3)=((1-dt*kk/2*Ek/Pm)*a1o(k1,k2,k3)+dt*cg1(k1,k2,k3))/(1+dt*kk/2*Ek/Pm)
   a2(k1,k2,k3)=((1-dt*kk/2*Ek/Pm)*a2o(k1,k2,k3)+dt*cg2(k1,k2,k3))/(1+dt*kk/2*Ek/Pm)
   a3(k1,k2,k3)=((1-dt*kk/2*Ek/Pm)*a3o(k1,k2,k3)+dt*cg3(k1,k2,k3))/(1+dt*kk/2*Ek/Pm)
#endif
  enddo
 enddo
enddo
...
do j3=1,n3
 do j1=1,n1 
  write(1,'(6E15.6)') x1(j1), x3(j3), v1(j1,n2/2+1,j3), v2(j1,n2/2+1,j3), v3(j1,n2/2+1,j3), &
  &                   v1(j1,n2/2+1,j3)**2+v2(j1,n2/2+1,j3)**2+v3(j1,n2/2+1,j3)**2
 enddo
enddo
...
enr2=0.d0
enr3=0.d0
!$omp parallel do private(j3,j2,j1) reduction(+:enr1,enr2,enr3) schedule(dynamic)
do j3=1,n3
 do j2=1,n2
  do j1=1,n1
...
div=0.d0
!$omp parallel do private(k3,k2,k1) reduction(+:div) schedule(dynamic)
do k3=1,n3
 do k2=1,n2
  do k1=1,nc
...
!$omp parallel do private(k3,k2,k1) schedule(dynamic)
do k3=1,n3
 do k2=1,n2
  do k1=1,nc
...
vp2=0
vp3=0
!$omp parallel do private(k3,k2,k1) reduction(+:vp1,vp2,vp3) schedule(dynamic)
do k3=1,n3
 do k2=1,n2
  do k1=1,nc
